...
public class MetricsService {
  ...
  public void recordRequest(String nodeId, boolean success, double responseTime) {
    ...
    log.debug(
        "Recorded request for node {}: success={}, responseTime={}", nodeId, success, responseTime);
  }
  ...
  private double calculatePercentileResponseTime(int percentile) {
    double[] sorted = responseTimes.stream().mapToDouble(Double::doubleValue).sorted().toArray();
  ...
  public void cleanupOldMetrics() {
    LocalDateTime cutoff = LocalDateTime.now().minusDays(config.getMetrics().getRetentionDays());
