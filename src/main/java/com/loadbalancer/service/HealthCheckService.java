...
public class HealthCheckService {
  ...
  private HealthStatus performHealthCheck(StorageNode node) {
    String url =
        String.format("http://%s:%d/api/v1/actuator/health", node.getHostAddress(), node.getPort());
    ...
    try {
      ...
      metricsService.recordRequest(
          node.getContainerId().toString(), true, System.currentTimeMillis() - startTime);
    } catch (Exception e) {
      isHealthy = false;
      statusMessage = e.getMessage();
      metricsService.recordRequest(
          node.getContainerId().toString(), false, System.currentTimeMillis() - startTime);
    }
  ...
  private void handleHealthCheckFailure(StorageNode node) {
    ...
    if (failures >= config.getHealthCheck().getFailureThreshold()) {
      log.warn(
          "Node {} marked as INACTIVE after {} consecutive failures",
          node.getContainerId(),
          failures);
